...
**Opening Fully flexible / Weekly on Booking via channel settings API is blocked until a new invite includes channels scopes.** Do **not** re-invite unless live `/authentication/details` after exchange shows channels missing (it does) **and** inventory price writes are insufficient.
