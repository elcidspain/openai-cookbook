...
def offers(token: str, api_base: str, room_id: int, agent_code: str | None):
    ...
    status, body = call("GET", "/inventory/rooms/offers?" + urllib.parse.urlencode(params), token, api_base)
    return status, body
...
def active_payload(code: str):
    ...
    for room_id, (label, smoke_price, guests) in ROOMS.items():
        rows.append({
            ...
            "name": f"AUMARA AI Agent API Smoke - {label}",
...
def main() -> int:
    ...
    created = []
    evidence = {"schema": "aumara-agent-fixed-price-smoke-v1", "testNight": TEST_NIGHT, "rooms": {}, "secretLogged": False}
    try:
        status, response = call("POST", "/inventory/fixedPrices", token, api_base, active_payload(code))
        evidence["createHttpStatus"] = status
        if status != 201:
            raise RuntimeError(f"fixed price create failed HTTP {status}: {response!r}")
        current = list_prices(token, api_base)
        for room_id, (label, smoke_price, _) in ROOMS.items():
            matches = [r for r in current if int(r.get("roomId") or 0) == room_id and str(r.get("name") or "").startswith("AUMARA AI Agent API Smoke")]
            if len(matches) != 1:
                raise RuntimeError(f"room {room_id}: expected one smoke fixed price, got {len(matches)}")
            row = matches[0]; created.append(row)
            page = row.get("bookingPage") or {}; codes = row.get("agentCodes") or []
            if page.get("direct") is not False or page.get("agent") is not True or len(codes) != 1:
            ...
            public_prices, agent_prices = extract_prices(public_body), extract_prices(agent_body)
            evidence["rooms"][str(room_id)] = {
                "label": label, "fixedPriceId": row.get("id"), "agentOnlyReadBack": True,
                "publicOfferHttp": public_status, "agentOfferHttp": agent_status,
                "smokePriceSeenPublic": smoke_price in public_prices, "smokePriceSeenWithAgentCode": smoke_price in agent_prices,
            }
            if smoke_price in public_prices:
                raise RuntimeError(f"room {room_id}: agent-only smoke price leaked into public offers")
        evidence["agentIsolationVerified"] = all(v["agentOnlyReadBack"] and not v["smokePriceSeenPublic"] for v in evidence["rooms"].values())
    finally:
        if not created:
            current = list_prices(token, api_base)
            created = [r for r in current if int(r.get("roomId") or 0) in ROOMS and str(r.get("name") or "").startswith("AUMARA AI Agent API Smoke")]
        if created:
            cleanup_status, cleanup_body = call("POST", "/inventory/fixedPrices", token, api_base, dormant_payload(created))
            ...
            if cleanup_status != 201:
                ...
                raise RuntimeError(f"dormancy cleanup failed HTTP {cleanup_status}: {cleanup_body!r}")
            dormant = list_prices(token, api_base)
            ids = {int(r.get("id") or 0): r for r in dormant}
            evidence["dormantVerified"] = all(
                int(r["id"]) in ids and (ids[int(r["id"])].get("bookingPage") or {}).get("agent") is False
    ...
    print(json.dumps({"status": "PASS", "agentIsolationVerified": evidence.get("agentIsolationVerified"), "dormantVerified": evidence.get("dormantVerified"), "rooms": evidence.get("rooms")}))
    return 0 if evidence.get("agentIsolationVerified") and evidence.get("dormantVerified") else 3
