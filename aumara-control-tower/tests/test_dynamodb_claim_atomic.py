...
class DynamoClaimAtomicTests(unittest.TestCase):
    ...
    def test_shadow_workflow_tolerates_auth_drift_with_degraded_summary(self) -> None:
        ...
        self.assertIn('mkdir -p "$(dirname "$auth_error_log")"', workflow)
        self.assertIn(
            'grep -q "Beds24 authentication failed with HTTP status"',
            workflow,
        )
        self.assertRegex(workflow, r"python - <<'PY'\n\s+import datetime as dt")
    ...
    def test_airbnb_draft_prepare_workflow_applies_chalet_multi_inventory_payload(self) -> None:
        workflow = (
            ROOT.parent
            / ".github"
            / "workflows"
        ...
        self.assertIn("if not api or not prop: raise SystemExit('Beds24 V1 content credentials missing in Production')", workflow)
